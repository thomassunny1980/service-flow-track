...
const InvoicePaymentDialog = ({ 
  ...
  const calculateBalance = () => {
    return totalAmount - calculateNewTotal();
  };

  const getPaymentStatus = () => {
    const newTotal = calculateNewTotal();
    if (newTotal >= totalAmount) return "paid";
  ...
  const handleSubmit = async () => {
    ...
    if (newTotal > totalAmount) {
      toast.error("Payment amount exceeds the invoice total");
      return;
    }
        ...
        <div className="space-y-4">
          <div className="p-4 bg-muted rounded-md space-y-2">
            ...
            <div className="flex justify-between border-t pt-2">
              ...
              <span className="font-medium text-destructive">
                ₹{(totalAmount - currentAmountPaid).toFixed(2)}
              </span>
            </div>
          </div>
          ...
          <div>
            ...
            <Input
              ...
              step="0.01"
              min="0"
              max={totalAmount - currentAmountPaid}
              placeholder="0.00"
